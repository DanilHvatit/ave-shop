...
function sectors(scr) {
	...
	var h = $(window).outerHeight();
	var headerheight = 80;
	var header = ($('header').next().outerHeight() + $('header').outerHeight());
...
$(document).ready(function () {
	...
	if (!isMobile.any()) {
		$.each($('.zoom'), function (index, val) {
			$(this).append('<span class="zoom__body"></span><span class="zoom__lens"></span><span class="zoom__big"></span>');
			...
			var body = $(this).find('.zoom__body');
			big.css({
				top: 0,
				height: $('.slider-view').outerHeight(),
				left: 0,
				width: $('.slider-view').outerWidth(),
				position: 'absolute'
...
if ($('.slider-view').length > 0) {
	...
	$('.slider-nav').slick({
		...
		// slide: $('.slider-nav__item').next(),
		// centerMode: true,
		// centerPadding: '0px',
		focusOnSelect: true,
		nextArrow: '<button type="button" class="slick-next"></button>',
		prevArrow: '<button type="button" class="slick-prev disable"></button>',
	});
	$('.slider-nav').on('beforeChange', function (slick, currentSlide, nextSlide) {
		$('.slick-prev').removeClass('disable');
		if ((currentSlide.currentSlide) == ((currentSlide.slideCount) - 2)){
			$('.slick-next').addClass('disable');
		} else {
			$('.slick-next').removeClass('disable');
		}
		if ($('.slick-prev').hasClass('slick-disabled') & (currentSlide.currentSlide == 1)) {
			$('.slick-prev').addClass('disable');
		} else {
			$('.slick-prev').removeClass('disable');
		}
	});
...
$('body').on('click', '.tab__navitem', function (event) {
	var eq = $(this).closest('.address-item').index();
	if (!$(this).hasClass('active')) {
		$(this).closest('.tabs').find('.tab__navitem').removeClass('active');
		$(this).addClass('active');
		$('.contact').closest('.tab-content').find('.tab__item').removeClass('active').eq(eq).addClass('active');
	}
});

$('body').on('click', '.discription__navlink', function (event) {
	var eq = $(this).index();

	if (!$(this).hasClass('active')) {
		$(this).closest('.discription').find('.discription__navlink').removeClass('active');
		$(this).addClass('active');
		$(this).closest('.discription').find('.discription__item').removeClass('active').eq(eq).addClass('active');
	}
});
...
$('.filter__item').click(function (event) {
	...
	if (i) {
		var port = '.products__item.f_' + i;
		$('.products__item').fadeOut(500);
		$(port).fadeIn(1000);
